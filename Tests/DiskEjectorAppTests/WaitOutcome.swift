...
/// ## ⚠️ 两个数字的口径完全不同，别混用
///
/// - ``polls``：**条件被求值的次数**。条件成立得快，拍数就少，哪怕墙钟被调度拖得很长。
///   实测正常时 1–3 拍。⇒ **判「是不是并发争抢」看这个**。
///   ⚠️ **「与机器负载无关」是有前提的**：只在「循环的迭代次数由**条件**决定」时成立。
///   一旦**退出由截止时间决定**（条件在整个窗口里始终不成立，或探针按脚本给答案），
///   `polls` 就变成**负载相关** —— 两次求值之间那次 `await`（`Task.sleep` / actor 跳转）
///   只保证**下界**：2026-09-21 CI 实测一次 `Task.sleep(50ms)` 拖到 ~6.1s，把 `timeout: 5`
///   的窗口整个吃掉，第 3 拍**永远没发生**（实得 `polls: 2`）。见 §8.118。
...
/// 把人指向正确的方向**，不是当门槛。
///
/// ⚠️ **推论（2026-09-21 实测）**：既然 `polls` 不是门槛，**守卫里就不许写
/// 「第 N 拍才成立」（N ≥ 3）** —— 那是把「机器的调度延迟」写进判据，
/// 于是**守卫自己 flaky**（这条踩过：§8.118）。写「第 2 拍成立」是与调度无关的：
/// 条件在第 1 拍为假时，「循环里那一拍」与「超时后那次补查」都会在第 **2** 次求值时看到它。
